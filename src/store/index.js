...
export default createStore({
  ...
  mutations: {
    setUser(state, userData) {
      ...
      document.cookie = `jwt=${userData.token}; path=/; max-age=${60 * 60 * 24 * 10}; SameSite=Strict`
    },
    clearUser(state) {
      state.user = null
      state.token = null
  ...
  actions: {
    ...
    async fetchTodos({ commit }) {
      try {
        const response = await axios.get('/todo/fetch', { withCredentials: true })
    ...
    async createTodo({ commit, state }, todoText) {
      try {
        ...
        const response = await axios.post('/todo/create', todoData, { withCredentials: true })
    ...
    async updateTodo({ commit, state }, { id, text, completed }) {
      try {
        ...
        const response = await axios.put(`/todo/update/${id}`, todoData, { withCredentials: true })
        commit('updateTodo', response.data.updatedTodo || { _id: id, text, completed })
        return response
      } catch (error) {
        throw error
      }
